...
const ThresholdMode THRESHOLD_MODE = MODE_QTY_ONLY; // <-- change here
...
// ---- Silence button ----
bool silenceButtonState = HIGH;
bool lastSilenceButtonState = HIGH;

unsigned long silencePressStart = 0;
const unsigned long SILENCE_HOLD_TIME = 3000;

unsigned long lastSilenceDebounce = 0;
const unsigned long debounceSilenceDelay = 20;

bool silenceHoldTriggered = false;

// ---- WiFi info display ----
bool wifiInfoActive = false;
unsigned long wifiInfoUntil = 0;
...
void loop() {
  ...
  // Keep WiFi info on screen while active.
    // LCD refresh timer
  if (
    !wifiInfoActive &&
    reweighState == REWEIGH_IDLE &&
  ...
  if (wifiInfoActive) {
    ...
    // Don't touch the LCD here.
    // The screen was already drawn by showWiFiInfo().

    if (millis() >= wifiInfoUntil) {

      wifiInfoActive = false;

      lcd.clear();

      // Restore whatever should currently be displayed
      if (reweighState == REWEIGH_IDLE) {

        if (millis() < messageScreenUntil) {
...
void handleSilenceButton() {
  ...
  bool reading = digitalRead(SILENCE_BTN);
  ...
  // Accept the new state after debounce time
  if (millis() - lastSilenceDebounce > debounceSilenceDelay) {
    ...
    if (reading != silenceButtonState) {
      ...
      if (silenceButtonState == LOW) {
        ...
        silencePressStart = millis();
        silenceHoldTriggered = false;
      ...
      // -----------------------------
        ...
        // Only do the normal mute toggle
        // if the 3-second hold was NOT triggered.
        if (!silenceHoldTriggered) {
          toggleSilenceState();
        }
      }
    }
  ...
  // -----------------------------
  if (
    silenceButtonState == LOW &&
    !silenceHoldTriggered &&
    millis() - silencePressStart >= SILENCE_HOLD_TIME
  ) {

    silenceHoldTriggered = true;

    showWiFiInfo();
  }
}

void toggleSilenceState() {
...
void showWiFiInfo() {
  ...
  wifiInfoActive = true;
  wifiInfoUntil = millis() + 3000;

  // Draw ONLY ONCE.
  drawWiFiInfo();

  Serial.println("WiFi info requested.");
}
